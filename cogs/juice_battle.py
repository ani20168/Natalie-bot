...
class JuiceBattle(commands.Cog):
    ...
    def __init__(self, client: commands.Bot):
        ...
        self.enhancement_protection_base_cost = 10000
        self.enhancement_protection_cost_step = 10000
        self.enhancement_offset_keys = (
            "hp_offset",
            "atk_offset",
            "def_offset",
            "agi_offset",
        )
        self.enhancement_offset_labels = {
            "hp_offset": "生命",
            "atk_offset": "攻擊",
            "def_offset": "防禦",
            "agi_offset": "敏捷",
        ...
        self.bleed_trigger_stacks = 3
        self.blood_explosion_damage = 6
        self.offset_blessing_multiplier = 2
        self.izanagi_blessing_duration = 3
        self.intimidation_duration = 2
        self.characters = {
            "ownerless": {
                "name": "一無所有者",
                "hp": 10,
            ...
            },
            {
                ...
                "abilities": [
                    ...
                    },
                    {
                        ...
                        "phase": "passive",
                        "cd": None,
                        "description": "本次攻擊成功時，賦予對方中毒效果，對方回合開始時 -1 HP，持續 3 回合。",
                    },
                ],
            },
    ...
    def build_tower_monster(self, floor: int) -> dict:
        ...
        return {
            ...
            "attack_offset_blessing_remaining": 0,
            "def_offset_multiplier": 1,
            "def_offset_blessing_remaining": 0,
            "intimidation_remaining": 0,
        }
    ...
    def tower_prepare_skill_cooldowns(self, fighter: dict, phase: str):
        ...
        Args:
            ...
            phase (str): "attack 或 defend"
        """
        sources = ("character", "weapon", "armor")
    ...
    async def prepare_for_restart(self) -> None:
        """
        重啟準備階段收場：爬塔暫停介面（checkpoint 不變），挑戰對戰取消並退還賭注。
        """
        ...
        for view in list(self.restart_views):
            try:
                if isinstance(
                    ...
                    await self.restart_pause_tower_view(view, handled_message_ids=handled_message_ids)
    ...
    async def restart_pause_tower_view(
        ...
        *,
        ...
        """
        重啟準備階段暫停爬塔介面（checkpoint 僅在通關一層時寫入 DB，此處不再額外保存）。
            ...
            for session in sessions:
                ...
                try:
                    ...
                    if is_challenge and bet > 0 and not session.get("vs_bot"):
                        ...
                        description = "機器人重啟，爬塔介面已失效；已通關樓層的 checkpoint 仍保留，請重新使用指令從挑戰前畫面繼續。"
                    else:
    ...
    def equipment_enhancement_level(self, entry: dict | None) -> int:
        """
        取得裝備強化等級；舊資料沒有欄位時視為 +0。

        Args:
            entry (dict | None): "背包中的裝備實例"

        Returns:
            level (int): "0 到目前強化上限的整數等級"
        """
        if not isinstance(entry, dict):
            return 0
        try:
            level = int(entry.get("enhancement_level", 0) or 0)
        except (TypeError, ValueError):
            level = 0
        return max(0, min(self.enhancement_max_level, level))

    def equipment_protection_trigger_count(self, entry: dict | None) -> int:
        """
        取得同一件裝備已觸發保護石的次數。
        ...
        Returns:
            count (int): "非負的保護石觸發次數"
        """
        if not isinstance(entry, dict):
            return 0
        try:
            count = int(entry.get("enhancement_protection_count", 0) or 0)
        except (TypeError, ValueError):
            count = 0
        return max(0, count)

    def equipment_protection_cost(self, entry: dict | None) -> int:
        """
        計算該件裝備下一次使用保護石的費用。
        ...
        Returns:
            cost (int): "下一次保護石費用"
        """
        count = self.equipment_protection_trigger_count(entry)
        return self.enhancement_protection_base_cost + (
            count * self.enhancement_protection_cost_step
        )

    def equipment_display_name(self, entry: dict | None) -> str:
        """
        組出含強化等級的裝備名稱。
        ...
        base_name = template["name"] if template is not None else str(entry.get("item_id") or "未知裝備")
        return f"{base_name} +{self.equipment_enhancement_level(entry)}"
    ...
    def serialize_equipment_instance(self, entry: dict) -> dict | None:
        ...
        return {
            ...
            "enhancement_level": self.equipment_enhancement_level(entry),
            "enhancement_protection_count": self.equipment_protection_trigger_count(entry),
        }
    ...
    def apply_poison(self, target: dict) -> bool:
        """
        ...
        Returns:
            applied (bool): "是否成功施加中毒；具備中毒免疫時為 False"
        """
        target_abilities = target.get("abilities")
        if not isinstance(target_abilities, list):
            target_abilities = self.normalize_abilities(target.get("ability"))
        if any(
            isinstance(ability, dict) and ability.get("id") == "poison_immunity"
    ...
    def roll_stat(self, base: int, offset: int, *, offset_multiplier: int = 1) -> tuple[int, int]:
        """
        擲 1d6 並加上基礎值與偏移。
        ...
        Args:
            base (int): "3"
            offset (int): "2"
            offset_multiplier (int): "偏移倍率，例如伊邪那岐祝福為 2"

        Returns:
            result (tuple[int, int]): "(dice, total) 例如 (3, 8)"
        """
        dice = random.randint(1, 6)
        return dice, dice + base + (offset * offset_multiplier)
    ...
    def roll_attack(self, attacker: dict, *, use_dodge_roll: bool) -> tuple[int, int, str]:
        ...
        """
        attack_offset_multiplier = int(attacker.get("attack_offset_multiplier", 1))
        if use_dodge_roll:
            dice, total, _text = self.tower_roll(
                attacker,
                attacker["agi"],
                attacker.get("agi_offset", 0),
                offset_multiplier=attack_offset_multiplier,
            )
    ...
    async def tower_begin_session(self, interaction: discord.Interaction, progress: dict, *, edit_message: bool):
        ...
        """
        progress = self.tower_normalize_loaded_progress(progress)
        view = JuiceBattleTowerFloorView(cog=self, progress=progress)
        embed = self.tower_floor_embed(view.progress)
        if edit_message:
    ...
    def build_bag_embed(self, juice_battle: dict, page: int, discard_confirm_slot: int | None = None) -> Embed:
        ...
        for index in range(start, end):
            ...
            value = (
                ...
                f"生命偏移 {offsets['hp_offset']:+d}｜攻擊 {offsets['atk_offset']:+d}｜"
                f"防禦 {offsets['def_offset']:+d}｜敏捷 {offsets['agi_offset']:+d}"
            )
            ...
            embed.add_field(name=f"[{slot_label}] {self.equipment_display_name(entry)}", value=value, inline=False)
        if discard_confirm_slot is not None:
            ...
            if isinstance(entry, dict):
                ...
                if template is not None:
                    name = self.equipment_display_name(entry)
            embed.add_field(
                name="丟棄確認",
    ...
    def build_enhance_embed(self, juice_battle: dict, protection_enabled: bool = False) -> Embed:
        ...
        bag = juice_battle.get("bag") or []
        for kind, slot_key, kind_text in (
            ("weapon", "equipped_weapon_slot", "武器"),
            ("armor", "equipped_armor_slot", "防具"),
        ):
            slot = juice_battle.get(slot_key)
            entry = bag[slot] if isinstance(slot, int) and 0 <= slot < len(bag) else None
            template = self.item_template(kind, entry.get("item_id")) if isinstance(entry, dict) else None
            if template is None:
                embed.add_field(name=kind_text, value="目前沒有裝備。", inline=False)
                continue
            offsets = self.equipment_stat_offsets(entry) or {}
            level = self.equipment_enhancement_level(entry)
            value_lines = [
                f"名稱：**{self.equipment_display_name(entry)}**",
                f"等級：**+{level}**",
                (
                    f"偏移：生命 {offsets['hp_offset']:+d}｜攻擊 {offsets['atk_offset']:+d}｜"
                    f"防禦 {offsets['def_offset']:+d}｜敏捷 {offsets['agi_offset']:+d}"
                ),
            ]
            if level >= self.enhancement_max_level:
                value_lines.append(f"已達最高強化等級 **+{self.enhancement_max_level}**。")
            else:
    ...
    def build_battle_embed(self, view: "JuiceBattleView") -> Embed:
        ...
        for fighter in (view.fighter_a, view.fighter_b):
            ...
            if int(fighter.get("bleed_stacks", 0)) > 0:
                status_parts.append(f"流血 {fighter['bleed_stacks']}")
            if fighter.get("stun_remaining", 0) > 0:
                status_parts.append("暈眩")
            if int(fighter.get("attack_offset_blessing_remaining", 0)) > 0:
                status_parts.append(
                    f"攻擊偏移×{int(fighter.get('attack_offset_multiplier', 1))}"
                    f"（{fighter['attack_offset_blessing_remaining']}回合）"
                )
            if int(fighter.get("def_offset_blessing_remaining", 0)) > 0:
                status_parts.append(
                    f"防禦偏移×{int(fighter.get('def_offset_multiplier', 1))}"
                    f"（{fighter['def_offset_blessing_remaining']}回合）"
                )
            if int(fighter.get("intimidation_remaining", 0)) > 0:
    ...
    @app_commands.command(name="juice_battle_enhance", description="強化目前裝備中的 Juice Battle 武器或防具")
    async def juice_battle_enhance(self, interaction: discord.Interaction):
        """
        開啟目前武器與防具的強化介面。
    ...
    def tower_normalize_loaded_progress(self, progress: dict) -> dict:
        """
        讀取 checkpoint 時移除不持久化的暫存欄位（舊版 battle／monster 等）。

        Args:
            progress (dict): "從 MongoDB 讀出的 tower_progress"

        Returns:
            progress (dict): "僅保留 checkpoint 欄位的副本"
        """
        normalized = copy.deepcopy(progress)
        normalized.pop("battle", None)
        normalized.pop("monster", None)
        return normalized

    def tower_sanitize_progress(self, progress: dict) -> dict:
        """
        整理要寫入 MongoDB 的爬塔 checkpoint（僅通關一層時保存）。

        Args:
            progress (dict): "記憶體中的爬塔快照"

        Returns:
            progress (dict): "可寫入 tower_progress 的精簡資料"
        """
        return {
            "member_ids": [str(member_id) for member_id in progress.get("member_ids") or []],
            "member_names": copy.deepcopy(progress.get("member_names") or {}),
            "floor": max(self.tower_floor_min, int(progress.get("floor", self.tower_floor_min))),
            "cleared_floor": max(0, int(progress.get("cleared_floor", 0))),
            "pending_cake": int(progress.get("pending_cake", 0)),
            "pending_equipment": [str(item_id) for item_id in progress.get("pending_equipment") or []],
            "member_hp": copy.deepcopy(progress.get("member_hp") or {}),
            "dead_ids": [str(member_id) for member_id in progress.get("dead_ids") or []],
            "member_loadouts": copy.deepcopy(progress.get("member_loadouts") or {}),
        }
    ...
    async def tower_load_shared_progress(self, member_ids: list[str]) -> dict | None:
        ...
        for member_id in member_ids:
            ...
            if stored_ids == expected_ids:
                return self.tower_normalize_loaded_progress(progress)
        return None
    ...
    async def tower_find_progress_for_user(self, userid: str) -> dict | None:
        ...
        user_data = await self.load_user(userid)
        progress = user_data["juice_battle"].get("tower_progress")
        return self.tower_normalize_loaded_progress(progress) if isinstance(progress, dict) else None

    async def tower_save_progress(self, progress: dict):
        """
        將同一份爬塔 checkpoint 同步寫入所有隊員（僅應在通關一層時呼叫）。
        ...
        if self.tower_is_aborted(member_ids):
            return
        sanitized = self.tower_sanitize_progress(progress)
        for member_id in member_ids:
            user_data = await self.load_user(member_id)
            user_data["juice_battle"]["tower_progress"] = copy.deepcopy(sanitized)
    ...
    def tower_floor_embed(self, progress: dict, *, log_text: str = "") -> Embed:
        ...
        """
        floor = int(progress.get("floor", self.tower_floor_min))
        monster = progress.get("monster") if isinstance(progress.get("monster"), dict) else self.build_tower_monster(floor)
        abilities = monster.get("abilities")
        if not isinstance(abilities, list):
            abilities = self.normalize_abilities(monster.get("ability"))
        ...
        if abilities:
            embed.add_field(
                name="技能",
                value="\n".join(
                    f"**{ability.get('name', '被動')}**：{ability.get('description') or '—'}"
                    for ability in abilities
    ...
    def tower_failure_embed(self, progress: dict, reason: str) -> Embed:
        """
        建立爬塔失敗回覆 embed，顯示失敗樓層與參戰隊伍。

        Args:
            progress (dict): "失敗時的爬塔進度快照"
        ...
        """
        floor = int(progress.get("floor", self.tower_floor_min))
        party_names = self.tower_party_names(progress)
        embed = Embed(
        ...
        embed.add_field(
            name="參戰隊伍",
            value="、".join(party_names) if party_names else "未知隊伍",
            inline=False,
        )
        return embed
        ...
        if view.message is not None:
            try:
                await view.message.edit(
                    embed=self.tower_failure_embed(view.progress, reason),
                    view=None,
                )
            except Exception:
                pass
    ...
    async def tower_start_battle(self, interaction: discord.Interaction, progress: dict):
        ...
        floor = int(progress.get("floor", self.tower_floor_min))
        monster = copy.deepcopy(
            progress.get("monster") if isinstance(progress.get("monster"), dict) else self.build_tower_monster(floor)
        )
    ...
    async def juice_battle_tower(self, interaction: discord.Interaction, teammate: discord.Member | None = None):
        ...
        async with self.tower_lock:
            ...
            if isinstance(own_progress, dict):
                ...
                if teammate is None:
                    ...
                    if set(member_ids) != set(stored_ids):
                        ...
                        return
                progress = self.tower_normalize_loaded_progress(own_progress)
            else:
            ...
            if start_selection_floor is None:
                # 新開局僅在記憶體補齊角色／裝備快照，checkpoint 待通關一層後才寫入 DB
...
class JuiceBattleEnhanceButton(discord.ui.Button):
    """強化武器或防具按鈕。"""

    def __init__(self, *, kind: str, disabled: bool):
        """
        建立指定部位的強化按鈕。
        ...
        """
        label = "強化武器" if kind == "weapon" else "強化防具"
        super().__init__(label=label, style=discord.ButtonStyle.success, row=0, disabled=disabled)
        self.kind = kind
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        await view.on_enhance(interaction, self.kind)
...
class JuiceBattleEnhanceView(discord.ui.View):
    ...
    def __init__(
        ...
        *,
        ...
        for kind, slot_key in (
            ...
            slot = juice_battle.get(slot_key)
            entry = bag[slot] if isinstance(slot, int) and 0 <= slot < len(bag) else None
            has_equipment = self.cog.item_template(
                kind,
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        ...
        if str(interaction.user.id) == self.userid:
            return True
        await interaction.response.send_message(
            embed=Embed(title="Juice Battle", description="這不是你的裝備強化介面。", color=common.bot_error_color),
            ephemeral=True,
        )
        return False
    ...
    async def on_enhance(self, interaction: discord.Interaction, kind: str):
        """
        扣除費用並結算指定部位的一次強化。
        ...
        async with common.jsonio_lock:
            ...
            if template is None:
                await interaction.response.send_message(
                    embed=Embed(title="Juice Battle", description=f"目前沒有可強化的{kind_text}。", color=common.bot_error_color),
                    ephemeral=True,
                )
                return
            level = self.cog.equipment_enhancement_level(entry)
            if level >= self.cog.enhancement_max_level:
                await interaction.response.send_message(
                    embed=Embed(
                        title="Juice Battle",
                        description=f"{kind_text}已達最高強化等級 +{self.cog.enhancement_max_level}。",
                        color=common.bot_error_color,
                    ),
                    ephemeral=True,
                )
            ...
            if cake < total_cost:
                await interaction.response.send_message(
                    embed=Embed(
                        title="Juice Battle",
                        description=(
                            f"{common.cake_emoji}不足，無法強化{kind_text}。"
                            f"\n需要 **{total_cost}**，目前只有 **{cake}**。"
                        ),
                        color=common.bot_error_color,
                    ),
                    ephemeral=True,
            ...
            user_data["cake"] = cake - total_cost
            selected_offset_key = random.choice(self.cog.enhancement_offset_keys)
            selected_offset_label = self.cog.enhancement_offset_labels[selected_offset_key]
            success = random.random() < self.cog.enhancement_success_rates[level]
            if success:
                entry["enhancement_level"] = level + 1
                offsets = self.cog.equipment_stat_offsets(entry) or {}
                entry[selected_offset_key] = offsets[selected_offset_key] + 1
                result_text = (
                    f"{kind_text}強化成功！達到 **+{level + 1}**，"
                    f"{selected_offset_label}偏移 **+1**。"
                )
            elif use_protection:
                trigger_count = self.cog.equipment_protection_trigger_count(entry)
                entry["enhancement_protection_count"] = trigger_count + 1
                result_text = f"{kind_text}強化失敗，保護石生效，未受到偏移量懲罰。"
            else:
                offsets = self.cog.equipment_stat_offsets(entry) or {}
                entry[selected_offset_key] = offsets[selected_offset_key] - 1
                result_text = (
                    f"{kind_text}強化失敗，{selected_offset_label}偏移 **-1**。"
                )
            await common.mongo_storage.replace_user(self.userid, user_data)
...
class JuiceBattleView(discord.ui.View):
    ...
    def prepare_attack_phase(self) -> bool:
        ...
        # 攻擊偏移祝福回合遞減
        attack_blessing_remaining = int(attacker.get("attack_offset_blessing_remaining", 0))
        if attack_blessing_remaining > 0:
            attack_blessing_remaining -= 1
            attacker["attack_offset_blessing_remaining"] = attack_blessing_remaining
            if attack_blessing_remaining <= 0:
                attacker["attack_offset_multiplier"] = 1
    ...
    def prepare_defend_phase(self):
        ...
        # 防禦偏移祝福回合遞減
        def_blessing_remaining = int(defender.get("def_offset_blessing_remaining", 0))
        if def_blessing_remaining > 0:
            def_blessing_remaining -= 1
            defender["def_offset_blessing_remaining"] = def_blessing_remaining
            if def_blessing_remaining <= 0:
                defender["def_offset_multiplier"] = 1
    ...
    async def execute_attack(self, interaction: discord.Interaction | None):
        ...
        if not is_followup:
            ...
            if "izanagi_blessing" in ability_ids:
                attacker["attack_offset_multiplier"] = self.cog.offset_blessing_multiplier
                attacker["attack_offset_blessing_remaining"] = self.cog.izanagi_blessing_duration
            if "intimidation" in ability_ids:
                defender["intimidation_remaining"] = self.cog.intimidation_duration
        ...
        # 震懾：僅普通攻擊（未發動攻擊階段技能、非連擊後續段）減半最終攻擊值
        if (
            not is_followup
            and int(attacker.get("intimidation_remaining", 0)) > 0
            and not any(ability.get("phase") == "attack" for ability in abilities)
        ):
            total = total // 2
            skill_note = f"{skill_note}（震懾減半）" if skill_note else "（震懾減半）"
    ...
    async def apply_defense_choice(self, interaction: discord.Interaction | None, *, mode: str, respond: bool):
        ...
        if not defender_stunned and mode == "defend":
            ...
            _dice, defense_total = self.cog.roll_stat(
                def_base,
                def_offset,
                offset_multiplier=def_offset_multiplier,
            )
            ...
            if attack_total >= dodge_total:
                ...
                if self.cog.fighter_has_ability(defender, "agile_position"):
                    before_hp = int(defender.get("hp", 0))
                    self.cog.tower_add_hp(defender, 1)
                    gained = int(defender.get("hp", 0)) - before_hp
                    if gained > 0:
                        outcome_line += f"，靈活身位 +{gained} HP"
        ...
        if self.pending_poison and actual_damage > 0:
            ...
            self.pending_poison = False
        # 出血：攻擊命中疊流血，三層血爆（不經防禦減傷）
        if actual_damage > 0 and self.cog.fighter_has_ability(attacker, "bleed"):
            defender["bleed_stacks"] = int(defender.get("bleed_stacks", 0)) + 1
            if int(defender["bleed_stacks"]) >= self.cog.bleed_trigger_stacks:
                bleed_actual = self.apply_incoming_damage(
                    defender,
                    self.cog.blood_explosion_damage,
                    absorbable=False,
                )
                defender["bleed_stacks"] = 0
                self.append_log(f"血爆造成 **{bleed_actual}** 點傷害")
                if defender["hp"] <= 0:
                    await self.finish_battle(
                        winner=attacker,
                        reason="血爆擊倒對手。",
                        interaction=interaction if respond else None,
                    )
                    return
                if attacker["hp"] <= 0:
                    await self.finish_battle(
        ...
        # 震懾：受影響者完成一次攻擊回合後遞減
        if int(attacker.get("intimidation_remaining", 0)) > 0:
            attacker["intimidation_remaining"] = max(0, int(attacker["intimidation_remaining"]) - 1)
        ...
        if next_attacker.get("is_bot"):
            # 先 ack 互動，再於同一 View 自動攻擊並一次更新訊息（避免先換 View 導致 bot 攻擊未執行）
            if respond and interaction is not None and not interaction.response.is_done():
                await interaction.response.defer()
            await self.run_bot_attack(None)
            return
    ...
    async def run_bot_attack(self, interaction: discord.Interaction | None):
        ...
        # 自動發動攻擊階段可用技能（角色／武器／防具；祭血術已立即結算，不進 armed）
        for source in ("character", "weapon", "armor"):
            for ability in self.cog.fighter_abilities(attacker, source):
                ability_id = str(ability.get("id") or "")
    ...
    async def accept_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        async with self.cog.tower_lock:
            ...
            if start_selection_floor is None:
                ...
                # 新開局／續爬補齊開局角色裝備快照（checkpoint 待通關一層後才寫入 DB）
...
class JuiceBattleTowerFloorView(discord.ui.View):
    ...
    def __init__(self, *, cog: JuiceBattle, progress: dict):
        ...
        super().__init__(timeout=cog.tower_view_timeout)
        self.cog = cog
        self.progress = cog.tower_normalize_loaded_progress(progress)
        floor = int(self.progress.get("floor", cog.tower_floor_min))
        self.progress["monster"] = cog.build_tower_monster(floor)
        self.message: discord.Message | None = None
        self.settled = False
        cog.track_restart_view(self)
        self.add_item(JuiceBattleTowerChallengeButton())
        if floor > cog.tower_floor_min:
...
class JuiceBattleTowerNextFloorView(discord.ui.View):
    ...
    async def on_timeout(self) -> None:
        """
        通關結果介面逾時後解除爬塔介面鎖定（checkpoint 已在通關時寫入）。
        """
        member_ids = [str(member_id) for member_id in self.progress.get("member_ids") or []]
        async with self.cog.tower_lock:
            if self.finished:
...
class JuiceBattleTowerView(discord.ui.View):
    ...
    def build_embed(self) -> Embed:
        ...
        monster_abilities = self.monster.get("abilities")
        if not isinstance(monster_abilities, list):
            monster_abilities = self.cog.normalize_abilities(self.monster.get("ability"))
        monster_status = []
        if int(self.monster.get("bleed_stacks", 0)) > 0:
            monster_status.append(f"流血 {self.monster['bleed_stacks']}")
        if self.monster.get("stun_remaining", 0) > 0:
            monster_status.append("暈眩")
        if int(self.monster.get("intimidation_remaining", 0)) > 0:
            monster_status.append(f"震懾（{self.monster['intimidation_remaining']}回合）")
        ...
        for fighter in self.fighters:
            status = []
            if int(fighter.get("bleed_stacks", 0)) > 0:
                status.append(f"流血 {fighter['bleed_stacks']}")
            if fighter.get("stun_remaining", 0) > 0:
                status.append("暈眩")
            if int(fighter.get("attack_offset_blessing_remaining", 0)) > 0:
                status.append(
            ...
            if int(fighter.get("intimidation_remaining", 0)) > 0:
                status.append(f"震懾（{fighter['intimidation_remaining']}回合）")
            if fighter.get("poison_remaining", 0) > 0:
    ...
    def prepare_player_turn(self, fighter: dict):
        ...
        if attack_blessing_remaining > 0:
            attack_blessing_remaining -= 1
            fighter["attack_offset_blessing_remaining"] = attack_blessing_remaining
            if attack_blessing_remaining <= 0:
                fighter["attack_offset_multiplier"] = 1
        # 中毒跳傷；翠毒披肩改為毒性回生
    ...
    def monster_abilities(self) -> list[dict]:
        """
        取得怪物的全部技能，並相容只有單一 ability 的舊資料。

        Returns:
            abilities (list[dict]): "怪物技能定義清單"
        """
        abilities = self.monster.get("abilities")
        if isinstance(abilities, list):
            return [ability for ability in abilities if isinstance(ability, dict)]
        ability = self.monster.get("ability")
        return [ability] if isinstance(ability, dict) else []

    def monster_ability(self, ability_id: str) -> dict | None:
        """
        依技能 ID 取得怪物技能。

        Args:
            ability_id (str): "rapid_fire"

        Returns:
            ability (dict | None): "技能定義；找不到時為 None"
        """
    ...
    def monster_skill_cooldown(self, ability_id: str) -> int:
        """
        取得怪物指定技能的剩餘 CD，並相容舊的單一 skill_cd 欄位。
        ...
        Returns:
            cd_left (int): "剩餘回合數"
        """
        skill_cds = self.monster.get("skill_cds")
        if isinstance(skill_cds, dict) and ability_id in skill_cds:
            return max(0, int(skill_cds.get(ability_id, 0)))
        primary_ability = self.monster.get("ability")
        if isinstance(primary_ability, dict) and primary_ability.get("id") == ability_id:
            return max(0, int(self.monster.get("skill_cd", 0)))
        return 0

    def monster_skill_ready(self, phase: str, ability_id: str | None = None) -> bool:
        """
        判斷怪物指定階段技能是否能使用。

        Args:
            phase (str): "attack 或 defend"
            ability_id (str | None): "rapid_fire；省略時使用該階段第一個 CD 技能"

        Returns:
            ready (bool): "是否可使用"
        """
        ability = self.monster_ability(ability_id) if ability_id else None
        if ability is None:
            for candidate in self.monster_abilities():
                if candidate.get("phase") == phase and candidate.get("cd") is not None:
                    ability = candidate
                    break
        if ability is None or ability.get("phase") != phase or ability.get("cd") is None:
            return False
        return self.monster_skill_cooldown(str(ability.get("id") or "")) <= 0

    def monster_consume_skill(self, ability_id: str | None = None):
        """
        消耗怪物指定的 CD 技能。

        Args:
            ability_id (str | None): "rapid_fire；省略時使用舊的主要技能"
        """
        if ability_id:
            ability = self.monster_ability(ability_id)
        else:
            ability = self.monster.get("ability") if isinstance(self.monster.get("ability"), dict) else None
        if ability is None or ability.get("cd") is None or not ability.get("id"):
            return
        skill_cds = self.monster.get("skill_cds")
        if not isinstance(skill_cds, dict):
            skill_cds = {}
            self.monster["skill_cds"] = skill_cds
        skill_cds[str(ability["id"])] = int(ability["cd"])
        self.monster["skill_cd"] = int(ability["cd"])

    def prepare_monster_skill_cooldowns(self):
        """
        在怪物回合開始時遞減所有 CD 技能。
        """
        skill_cds = self.monster.get("skill_cds")
        if not isinstance(skill_cds, dict):
            ...
            self.monster["skill_cds"] = skill_cds
        if skill_cds:
            for ability_id, cd_left in list(skill_cds.items()):
                skill_cds[ability_id] = max(0, int(cd_left) - 1)
            self.monster["skill_cd"] = min(skill_cds.values(), default=0)
        elif int(self.monster.get("skill_cd", 0)) > 0:
    ...
    def choose_monster_defense(self, attack_total: int, bound: bool) -> str:
        ...
        if (
            self.monster_skill_ready("defend", "sprint")
            and int(self.monster.get("agi", 0)) != 0
        ):
            self.monster["sprint_armed"] = True
            self.monster_consume_skill("sprint")
            return "dodge"
    ...
    def resolve_player_attack(self, attacker: dict, *, use_dodge_roll: bool, bound: bool) -> str:
        ...
        if mode == "stunned":
            ...
            damage = max(1, attack_total - defense_total)
            hardening_text = ""
            if self.monster_skill_ready("defend", "hardened_shell"):
                self.monster_consume_skill("hardened_shell")
                if damage <= 3:
                    damage = 0
    ...
    async def on_attack(self, interaction: discord.Interaction):
        ...
        if "izanagi_blessing" in ability_ids:
            for fighter in self.living_fighters():
                fighter["attack_offset_multiplier"] = self.cog.offset_blessing_multiplier
                fighter["attack_offset_blessing_remaining"] = self.cog.izanagi_blessing_duration
            results.append("伊邪那岐的祝福發動，我方攻擊偏移量倍增")
        if "intimidation" in ability_ids:
            self.monster["intimidation_remaining"] = self.cog.intimidation_duration
            results.append(f"{self.monster['name']} 受到震懾")
        ...
        for attack_index, plan_entry in enumerate(attack_plan):
            ...
            if condemn_damage is not None:
                ...
                condemn_damage = None
            if self.last_attack_damage > 0 and self.cog.fighter_has_ability(attacker, "bleed"):
                self.monster["bleed_stacks"] = int(self.monster.get("bleed_stacks", 0)) + 1
                if int(self.monster["bleed_stacks"]) >= self.cog.bleed_trigger_stacks:
                    bleed_actual = self.apply_damage(
                        self.monster,
                        self.cog.blood_explosion_damage,
                        absorbable=False,
                    )
                    self.monster["bleed_stacks"] = 0
                    results.append(f"血爆造成 **{bleed_actual}** 點傷害")
            if self.monster["hp"] <= 0:
                break
            if attacker.get("pending_poison") and self.last_attack_damage > 0:
                if self.cog.apply_poison(self.monster):
                    results.append(f"{self.monster['name']} 中毒")
                else:
                    results.append(f"{self.monster['name']} 中毒免疫")
                attacker["pending_poison"] = False
        attacker["pending_poison"] = False
        self.log_text = "\n".join(results)
        # 黏液只影響本次攻擊的閃避結算
    ...
    def prepare_monster_attack(self, target: dict, *, skill_note: str = "", prepare_defense: bool = True):
        """
        建立一次怪物攻擊，並切換到指定玩家的防守階段。

        Args:
            target (dict): "本次被攻擊的玩家"
            skill_note (str): "本次攻擊發動的技能名稱"
            prepare_defense (bool): "是否遞減並清除玩家防守技能狀態"
        """
        ...
        # 震懾：怪物普通攻擊（本段無攻擊技能）最終攻擊值減半
        if int(self.monster.get("intimidation_remaining", 0)) > 0 and not skill_note:
        ...
        if prepare_defense:
            ...
            # 防禦偏移祝福回合遞減
            def_blessing_remaining = int(target.get("def_offset_blessing_remaining", 0))
            if def_blessing_remaining > 0:
                def_blessing_remaining -= 1
                target["def_offset_blessing_remaining"] = def_blessing_remaining
                if def_blessing_remaining <= 0:
                    target["def_offset_multiplier"] = 1
        ...
        skill_text = f"（發動 {skill_note}）" if skill_note else ""
        self.append_log(f"{self.monster['name']} 攻擊 **{attack_total}**{skill_text}")
    ...
    async def run_monster_turn(self):
        ...
        if bind_ability is not None and self.monster_skill_ready("attack", "bind"):
            self.monster_consume_skill("bind")
            self.pending_bind = True
            attack_skill_names.append(bind_ability["name"])
        rapid_fire_ability = self.monster_ability("rapid_fire")
        if rapid_fire_ability is not None and self.monster_skill_ready("attack", "rapid_fire"):
            self.monster_consume_skill("rapid_fire")
            self.pending_monster_extra_attacks = 1
            attack_skill_names.append(rapid_fire_ability["name"])
        self.prepare_monster_attack(
            target,
            skill_note="、".join(attack_skill_names),
        )
        self.rebuild_buttons()
        if self.message is not None:
            await self.message.edit(embed=self.build_embed(), view=self)
    ...
    async def resolve_player_defense(self, mode: str, interaction: discord.Interaction):
        ...
        monster_skill_note = (
            ...
            if self.pending_bind and bind_ability is not None
            else ""
        )
        ...
        if not defender_stunned and mode == "dodge":
            ...
            if "izanami_blessing" in ability_ids:
                for fighter in self.living_fighters():
                    ...
                    fighter["def_offset_blessing_remaining"] = self.cog.izanagi_blessing_duration
                log_parts.append("伊邪那美的祝福發動，我方防禦偏移量倍增")
        ...
        if self.monster_ability("poison") is not None and actual_damage > 0:
            if self.cog.apply_poison(defender):
                log_parts.append(f"{defender['display_name']} 中毒")
            else:
                log_parts.append(f"{defender['display_name']} 中毒免疫")
        self.log_text = "\n".join(log_parts)
        self.pending_attack_total = None
        self.pending_attack_dice = ""
        self.pending_bind = False
        ...
        if self.monster["hp"] <= 0:
            await self.cog.tower_finish_floor(self, self.log_text)
            return
        # 連發弩：第一次防守結算後，保留同一怪物回合再攻擊一次
        if self.pending_monster_extra_attacks > 0:
            self.pending_monster_extra_attacks -= 1
            living = self.living_fighters()
            if not living:
                await self.cog.tower_finish_defeat(self, "所有我方成員都已死亡。")
                return
            next_target = defender if defender["hp"] > 0 else random.choice(living)
            self.pending_bind = False
            self.prepare_monster_attack(
                next_target,
                skill_note="連發弩",
                prepare_defense=next_target is not defender,
            )
            self.rebuild_buttons()
            if self.message is not None:
                await self.message.edit(embed=self.build_embed(), view=self)
        ...
        # 震懾：怪物完成一次攻擊回合後遞減
        if int(self.monster.get("intimidation_remaining", 0)) > 0:
            self.monster["intimidation_remaining"] = max(0, int(self.monster["intimidation_remaining"]) - 1)
        await self.enter_next_player()
        if interaction.response.is_done():
            return
        await interaction.response.edit_message(embed=self.build_embed(), view=self)
    ...
    async def on_timeout(self) -> None:
        """
        戰鬥操作逾時時解除介面鎖定；checkpoint 維持在上一層通關時的狀態。
        """
        async with self.cog.tower_lock:
            if self.finished:
                return
        ...
        try:
            if not show_timeout_embed:
                await self.message.edit(
                    embed=self.cog.tower_failure_embed(
                        self.progress,
                        "所有我方成員都已死亡。",
                    ),
                    view=None,
            ...
            embed = self.cog.tower_floor_embed(
                self.progress,
                log_text=(
                    "戰鬥操作逾時，請重新使用指令從本層挑戰前畫面繼續。"
                    if existing is not None
                    else "戰鬥操作逾時，且尚未通關任何一層，請重新開始爬塔。"
                ),
            )
            await self.message.edit(embed=embed, view=None)
        except Exception:
